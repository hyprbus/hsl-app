...
export function arrivalsReducer(
  state: IArrival[] = testArrivals,
  ...
  switch (action.type) {
    case FETCH_ARRIVALS_SUCCESS:
      ...
      const filtered = action.arrivals.filter(
        a =>
          a.scheduledArrival > currentTime &&
          a.scheduledArrival < currentTime + 20 * 60
      );
      const sortedArrivals: IArrival[] = sortArray(
        filtered,
        "scheduledArrival"
      );
...
export function fetchingArrivalsReducer(
  state: boolean = false,
  action: ArrivalAction
): boolean {
  switch (action.type) {
